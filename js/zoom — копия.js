var canvas1 = document.getElementById('cansiatka2');
	...
	window.onload = function(){		
		...
		function redraw2(){
			...
			var p2 = ctx2.transformedPoint(canvas2.width,canvas2.height);
		...
		var lastX=canvas2.width/2, lastY=canvas2.height/2;
		var dragStart,dragged;
		...
		canvas2.addEventListener('mousemove',function(evt){
			...
			dragged = true;
			if (dragStart){
				var pt = ctx2.transformedPoint(lastX,lastY);
	...
	// Adds ctx2.transformedPoint(x,y) - returns an SVGPoint
	function trackTransforms2(ctx2){
		...
		var pt  = svg.createSVGPoint();
		ctx2.transformedPoint = function(x,y){
			pt.x=x; pt.y=y;
			return pt.matrixTransform(xform.inverse());
		}
	}